...
import SwiftUI
        ...
        where T: Numeric & _FormatSpecifiable & Strideable, Label: View
    {
        ...
        private func decreaseAction(_ immediateAction: Bool) {
            Haptics.play(longStep != nil && immediateAction ? .immediateAction : .click)
            let nStep: T = {
                if immediateAction, let longStep {
                    return longStep
                }
                return step
            }()
            value = max(range.lowerBound, value - nStep)
        }

        private func increaseAction(_ immediateAction: Bool) {
            Haptics.play(longStep != nil && immediateAction ? .immediateAction : .click)
            let nStep: T = {
                if immediateAction, let longStep {
                    return longStep
            ...
            value = min(value + nStep, range.upperBound)
        }
    }
    ...
    struct GroutStepper_Previews: PreviewProvider {
        struct TestHolder: View {
            ...
            var body: some View {
                GroutStepper(value: $value, in: 0 ... 200, step: 5.0, longStep: 100) {
